...
skickar ut det som resultat. I exemplet är \mono{C}-flaggan satt till 1 sen
innan. För aritmetiska instruktioner använder sig ALU:n av en enda adderare men
med olika möjliga operander. I exemplet kommer \mono{op1} väljas och
subtraheras med ett eftersom \mono{C} är satt. Den första operanden till
adderaren blir då \mono{op1}-1. För den andra operanden väljs \mono{op2} och
dess negation eftersom \mono{sbc} utför en subtraktion.  Den andra operanden är
då \mono{-op2} och adderaren kommer ge \mono{op1}-1-\mono{op2} vilket är det
