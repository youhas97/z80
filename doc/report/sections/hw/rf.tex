...
Lösningen till det här är att översätta \mono{111} till \mono{110} och
\mono{110} till \mono{111} direkt i instruktionsdekodaren. På så sätt kan
\mono{A} och \mono{F} enkelt hanteras som antingen två 8-bitars register eller
ett 16-bitars register. Notera att vid instruktioner som refererar till
\mono{(hl)} eller i vårt fall \mono{F} kommer värdet inte att hämtas från
registerfilen och därmed används inte adressen. Adressen till \mono{F} används
endast vid \mono{push} och \mono{pop} eftersom där måste en byte i taget
